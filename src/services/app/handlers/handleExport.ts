...
import { getFramesCount, getCurrentFrameIndex, switchToFrame, toggleOnionSkinning } from "./handleFrames";
...
function startGifExport() {
    ...
    if (!window.GIF) {
        ...
        loadGifJsLibrary().then(() => {
            ...
            alert('Failed to load GIF export library. Please check your internet connection and try again.');
            if (startExportBtn) {
                startExportBtn.textContent = "Generate GIF";
                startExportBtn.disabled = false;
            }
...
async function captureFrames(currentFrame: number, totalFrames: number, delay: number, width: number, height: number): Promise<void> {
